#' @title inner_join
#' @description Inner join between two dfi.
#'
#' @param x A dfi.
#' @param y A dfi.
#' @param by Columns to join.
#' @param ... Other arguments, see inner_join from dplyr.
#'
#' @return Inner join result dfi.
#'
...
inner_join = function(x, y, by, ...){
  ...
  dataframe <- dplyr::inner_join(x$dataframe, y$dataframe, by = by, ...)
    ...
    if(str_col %in% str_l_cols_nj){
      ...
      new_lng[[length(new_lng)+1]] <-
        base::data.frame(
          ...
          ACTION =  base::paste("IJ", x$name, y$name),
