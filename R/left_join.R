#' @title left_join
#' @description Left join between two dfi.
#'
#' @param x A dfi.
#' @param y A dfi.
#' @param by Columns to join.
#' @param ... Other arguments, see left_join from dplyr.
#'
#' @return Left join result dfi.
#'
...
left_join = function(x, y, by, ...){
  ...
  dataframe <- dplyr::left_join(x$dataframe, y$dataframe, by = by, ...)
    ...
    if(str_col %in% str_l_cols_nj){
      ...
      new_lng[[length(new_lng)+1]] <-
        base::data.frame(
          ...
          ACTION =  base::paste("LJ", x$name, y$name),
